...
@Pipe({
  name: 'ultimosdigitoscpfcnpj'
})
export class UltimosDigitosCpfCnpjPipe implements PipeTransform {
  ...
  transform(numero: string): string {
    ...
    if (!numero || numero.length < 10) {
      ...
      if (identificacao.length === 11) {
        ...
        return identificacao.replace(/(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/g, "\$1.\$2.\$3\/\$4\-\$5").split('/')[1];
      }
